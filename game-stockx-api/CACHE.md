...
Unknown catalogue (`unknown=true`) checks the current database administrator role before any cache access. Its separate cache key and shared serial-existence predicate keep counts and cards consistent with the question-mark badge. Serial additions already increment the catalogue revision, removing identified games from Unknown on the next request.

Worldwide (region 8) matches every regional filter, including Other, in the catalogue and collections; distinct platform and owned totals include it in each group. Unknown responses include `region_counts` computed with all current catalogue filters except the region selection itself and pagination. Each count equals `total_count` for that single region; unions must not be calculated by adding these overlapping counts. Cache namespaces were advanced for the new semantics.
