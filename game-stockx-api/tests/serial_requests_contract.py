...
def exercise(base, admin, uploader, sql):
    ...
    sql("""UPDATE releases SET serial=ARRAY['CUSA-00123'] WHERE id=1;
    ...
    assert sql('SELECT serial::text FROM releases WHERE id=1') == '{CUSA-00123}'
    ...
    sql(f"UPDATE release_serial_requests SET serial='cusa12345' WHERE id={entry_id}")
    submit(status=409)
    sql(f"UPDATE release_serial_requests SET serial='CUSA-12345' WHERE id={entry_id}")
    ...
    assert pending['serial'] == 'CUSA-12345' and pending['existing_serials'] == ['CUSA-00123']
    ...
    assert request('/api/admin/serial-requests')['items'][0]['existing_serials'] == ['CUSA-00123','EXTERNAL-456']
    ...
    assert sql('SELECT serial::text FROM releases WHERE id=1') == '{CUSA-00123,EXTERNAL-456}'
    ...
    assert sql('SELECT serial::text FROM releases WHERE id=1') == '{CUSA-00123,EXTERNAL-456,CUSA-12345}'
    ...
    another = submit(serial='CUSA-90002')['id']
    sql("UPDATE releases SET serial=array_append(serial,'CUSA-90002') WHERE id=1")
    request(f'/api/admin/serial-requests/{another}/accept', method='POST', status=204)
    assert sql("SELECT count(*) FROM releases,unnest(serial) s WHERE id=1 AND s='CUSA-90002'") == '1'
    rejected = submit(serial='CUSA-90003')['id']
    ...
    assert 'CUSA-90003' not in sql('SELECT serial::text FROM releases WHERE id=1')
    ...
    edited_id = submit(serial='CUSA-90004')['id']
    ...
    accept_edit('CUSA-90005', status=403, token=uploader)
    request(f'/api/admin/serial-requests/{edited_id}/archive', method='DELETE', status=409)
    accept_edit('  cusa90005 ')
    assert 'CUSA-90004' not in sql('SELECT serial::text FROM releases WHERE id=1')
    assert 'CUSA-90005' in sql('SELECT serial::text FROM releases WHERE id=1')
    edited = next(row for row in request('/api/admin/serial-requests?status=accepted')['items'] if row['id'] == edited_id)
    assert edited['serial'] == 'CUSA-90005' and edited['submitted_serial'] == 'CUSA-90004'
    accept_edit('CUSA-90005')
    accept_edit('CUSA-90006', status=409)
    assert 'CUSA-90006' not in sql('SELECT serial::text FROM releases WHERE id=1')
    ...
    for number in range(20): submit(serial=f'CUSA-{80000+number}')
    submit(serial='CUSA-80021', status=409)
