...
def exercise(request, sql, admin, ordinary):
    ...
    for token,status in [(None,401),('bad',401),(ordinary,403)]:
        request(serial,token,data={'serial':'CUSA-987'},status=status)
    ...
    request('/api/admin/releases/999999/serials',admin,data={'serial':'CUSA-987'},status=404)
    ...
    request('/api/admin/releases/900/serials',admin,data={'serial':'SLUS-111'},status=400)
    ...
    request(serial,admin,data={'serial':' cusa-987 '},status=204)
    request(serial,admin,data={'serial':'CUSA-987'},status=409)
    assert sql('SELECT serial[1] FROM releases WHERE id=1')=='CUSA-987'
    ...
    request(serial,admin,data={'serial':'CUSA-ROLLBACK'},status=500)
    ...
    assert sql("SELECT count(*) FROM releases WHERE 'CUSA-ROLLBACK'=ANY(serial)")=='0'
