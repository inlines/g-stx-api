...
use diesel::prelude::*;
use diesel::sql_types::{Text, Integer, Nullable, BigInt, Array, Bool};
use actix_web::http::header;
use serde::{Deserialize, Serialize};
use crate::pagination::Pagination;
use crate::metrics::{SUCCESSFUL_ADD_TO_COLLECTION, SUCCESSFUL_ADD_TO_WISHLIST, SUCCESSFUL_ADD_TO_WTS};
...
#[get("/wts")]
async fn get_wts(pool: web::Data<DBPool>, req: HttpRequest, query: web::Query<Pagination>) -> HttpResponse {
    // Извлечение токена из заголовка
    let token = match req.headers().get(header::AUTHORIZATION) {
        Some(header_value) => {
            let header_str = header_value.to_str().unwrap_or("");
...
async fn add_release(
    ...
    match result {
        Ok(_) => { SUCCESSFUL_ADD_TO_WTS.inc(); HttpResponse::Ok().body({})},
        Err(err) => {
